...
extension PKSigning {
    ...
    internal func signUsingES256(_ challenge: Data, privateKey: String) -> Data? {
        ...
        do {
            ...
            print("Failed to parse as ES256 private key: \(message, privacy: .public)")
            return nil
        }
        ...
        do {
            ...
            let message = (error as NSError).debugDescription
            print("Failed to sign using ES256: \(message, privacy: .public)")
            return nil
        }
    }
    /**
    ...
    internal func signUsingEd25519(_ challenge: Data, privateKey: String) -> Data? {
        ...
        do {
            privateKey = try Curve25519.Signing.PrivateKey(pemRepresentation: privateKey)
        } catch {
            let message = (error as NSError).debugDescription
            print("Failed to parse as EdDSA private key: \(message, privacy: .public)")
            return nil
        }
        do {
            let signature = try privateKey.signature(for: challenge)
            ...
            let message = (error as NSError).debugDescription
            print("Failed to sign using EdDSA: \(message, privacy: .public)")
            return nil
        }
    }
}
